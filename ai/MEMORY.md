...
- Core purpose: tag payloads with PCRE2 named-capture patterns, optionally compute similarity hashes (TLSH/LZJD/feature-gated MRSHv2) and pairwise distances, emit JSON records to STDOUT and optional run stats to STDERR.
...
4. Decode payload (`base64`/`string`/`hex`) and optionally extract from JSON path.
...
## Known Constraints
- Blob mode (`--input-blob`) is implemented, but encoded blob decoding (`base64`/`hex`) currently expects UTF-8 wrapper text.
...
- Similarity backend support now includes:
  ...
  - `fbhash` (scaffolded for future work)
...
3. Evaluate library/CLI split (`src/lib.rs`) for embeddability.
4. Reduce stats-related overhead when `--stats` is disabled.
